...
**安装脚本修复**
- 修复 `运行环境安装.bat` / `安装maabot.bat` 在中文 Windows 下的乱码问题
- 改用 GBK 编码 + ASCII 边框字符，移除不可靠的 `chcp 65001`，兼容所有中文 Windows 系统
- 安装脚本统一放在 `maabot-0.24/` 目录

**安全加固**
- 移除代码中硬编码的 FRP 服务器 IP、WebUI 域名等敏感信息，改为从 config.yaml 读取
- WebUI 默认密码不再硬编码，首次运行时随机生成并打印到控制台
- WebUI 默认用户名改为 `admin`
- 完善 `.gitignore`，排除 `*.stackdump`、临时调试目录等
...
`maabot.py` 会自动从 `config.yaml` 读取这两个值，无需修改代码。如需修改 MAA 路径，编辑 `config.yaml` 中的 `maa_exe` / `maa_log_path` / `maa_config_path`。
